...
class Watcher(object):
    def __init__(self, config_dir):
        ...
        # The eww config dir (holds eww.yuck + the generated eww.theme.* files)
        # is a subdirectory of the widget root.
        self.eww_dir = os.path.join(self.config_dir, "eww")
        ...
        for attempt in range(1, attempts + 1):
            try:
                reload = subprocess.run(
                    ["eww", "--config", self.eww_dir, "reload"],
                    capture_output=True,
                    text=True,
                    timeout=20,
                )
    ...
    def _apply(self):
        ...
        try:
            gen = subprocess.run(
                [sys.executable, os.path.join(self.config_dir, "scripts", "core", "theme.py"), self.config_dir],
                capture_output=True,
                text=True,
                timeout=30,
            )
        ...
        if self.config_changed:
            ...
            log_path = os.path.join(self.config_dir, "logs", "watch.log")
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            try:
                with open(log_path, "a", encoding="utf-8") as logf:
                    relayout = subprocess.run(
                        [os.path.join(self.config_dir, "scripts", "bin", "start.sh"), "--relayout"],
                        stdout=logf,
