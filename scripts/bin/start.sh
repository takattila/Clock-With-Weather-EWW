...
# True when the widget was installed with INSTALL_METHOD=docker: a
# docker-start.sh helper exists in bin/ (generated by install-docker.sh) and/or
# EWW_CONTAINER is exported. In Docker mode the eww daemon + the watchers run
# INSIDE the container (owned by scripts/docker/entrypoint.sh); this script
# only drives the sync layout/geometry commands through the eww/python3 wrapper
# scripts, and the keyboard (input daemon) control is disabled.
detect_docker_mode() {
...
relayout() {
  # In Docker mode make sure the container is up first (idempotent) so the
  # eww close-all / layout wrapper calls reach a running daemon.
...
main() {
  ...
  if [[ "$DOCKER_MODE" = "true" ]]; then
    # Docker mode: the eww daemon + watchers are owned by the container
    # entrypoint (start_eww is NOT called here -- its `eww kill` would drop
    # the container's daemon). stop.sh above removed any previous container;
    # start it FIRST so the sync theme/layout wrappers (docker exec) reach a
    # running daemon, then drive the window layout through them.
    "$DIR/scripts/bin/docker-start.sh"
    generate_theme
    layout_windows
    echo "Clock + weather widget and system monitor panel are running (eww, Docker)."
    return
  fi
