...
The listener writes its PID to generated/esc_listener.pid. scripts/move_ctl.py
...
PIDFILE = os.path.join(CONFIG_DIR, "generated", "esc_listener.pid")
...
def main():
    ...
    def handle(key):
        if args.mode == "move":
            ...
            subprocess.run(
                ["python3", os.path.join(SCRIPT_DIR, "close_popup.py")],
                check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
        Gtk.main_quit()
