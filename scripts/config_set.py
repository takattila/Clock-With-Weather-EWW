...
are no global position/scale keys anymore). For the panel they are per-monitor
...
def block_lines(parent_indent, data):
    ...
    for monitor in sorted(data, key=lambda k: str(k)):
        ...
        for key, value in data[monitor].items():
            # The scale always gets two decimals (1.00 / 0.45 / 0.90), whatever
            # its current form (float 1.0, int 1 or string "1.00"), so a YAML
            # round-trip can never shorten 0.90 to 0.9 or 1.00 to 1.0.
            if key == "scale":
                try:
